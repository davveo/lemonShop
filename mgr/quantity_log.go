...
func NewQuantityLogMgr(db db.Repo) *QuantityLogMgr {
	...
	return &QuantityLogMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_quantity_log"), wdb: db.GetDbW().Table("es_quantity_log"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
