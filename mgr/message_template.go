...
func NewMessageTemplateMgr(db db.Repo) *MessageTemplateMgr {
	...
	return &MessageTemplateMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_message_template"), wdb: db.GetDbW().Table("es_message_template"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
