...
func NewWechatMsgTemplateMgr(db db.Repo) *WechatMsgTemplateMgr {
	...
	return &WechatMsgTemplateMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_wechat_msg_template"), wdb: db.GetDbW().Table("es_wechat_msg_template"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
