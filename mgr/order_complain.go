...
func NewOrderComplainMgr(db db.Repo) *OrderComplainMgr {
	...
	return &OrderComplainMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_order_complain"), wdb: db.GetDbW().Table("es_order_complain"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
