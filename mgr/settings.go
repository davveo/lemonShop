...
func NewSettingsMgr(db db.Repo) *SettingsMgr {
	...
	return &SettingsMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_settings"), wdb: db.GetDbW().Table("es_settings"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
