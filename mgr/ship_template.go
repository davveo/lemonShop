...
func NewShipTemplateMgr(db db.Repo) *ShipTemplateMgr {
	...
	return &ShipTemplateMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_ship_template"), wdb: db.GetDbW().Table("es_ship_template"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
