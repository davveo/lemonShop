...
func NewPaymentBillMgr(db db.Repo) *PaymentBillMgr {
	...
	return &PaymentBillMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_payment_bill"), wdb: db.GetDbW().Table("es_payment_bill"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
