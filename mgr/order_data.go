...
func NewSssOrderDataMgr(db db.Repo) *SssOrderDataMgr {
	...
	return &SssOrderDataMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_sss_order_data"), wdb: db.GetDbW().Table("es_sss_order_data"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
