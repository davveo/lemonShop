...
func NewMemberAddressMgr(db db.Repo) *MemberAddressMgr {
	...
	return &MemberAddressMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_member_address"), wdb: db.GetDbW().Table("es_member_address"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
