...
func NewHotKeywordMgr(db db.Repo) *HotKeywordMgr {
	...
	return &HotKeywordMgr{_BaseMgr: &_BaseMgr{rdb: db.GetDbR().Table("es_hot_keyword"), wdb: db.GetDbW().Table("es_hot_keyword"), isRelated: globalIsRelated, ctx: ctx, cancel: cancel, timeout: -1}}
}
