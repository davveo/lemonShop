...
func NewRabbitMQ() *RabbitMQ {
	url := "amqp://" + conf.Conf.RabbitMQ.UserName + ":" +
		conf.Conf.RabbitMQ.Password + "@" +
		conf.Conf.RabbitMQ.Host + ":" +
		conf.Conf.RabbitMQ.Port + "/"
...
// ConsumeInit 初始化消费服务 直接发送消息，direct模式，路由设置与队列名一致
func (rabbitMQ *RabbitMQ) ConsumeInit(exchangeName string, queueName string) error {
	...
	if err := rabbitMQ.channel.ExchangeDeclare(
		...
		rabbitMQ.close()
		logger.GLogger.Error("绑定exchange异常" + err.Error())
		return err
	}
	...
	if _, err := rabbitMQ.channel.QueueDeclare(
		...
		rabbitMQ.close()
		logger.GLogger.Error("创建queue异常" + err.Error())
		return err
	}
	...
	if err := rabbitMQ.channel.QueueBind(
		...
		fmt.Println("绑定queue异常：", err.Error())
		rabbitMQ.close()
		logger.GLogger.Error("绑定queue异常" + err.Error())
		return err
	}
